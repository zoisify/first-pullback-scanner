...
def get_alpaca_client():
    ...
    secret = __import__("os").environ["ALPACA_SECRET_KEY"]
    return StockHistoricalDataClient(key, secret), TradingClient(key, secret, paper=True)


def get_float(ticker: str) -> int | None:
    """Get reported public float from SEC EDGAR, with a labelled proxy fallback."""
    try:
        ...
        if value and int(value) > 0:
            print(
                f"  [{ticker}] Float proxy: {int(value):,} "
...
def get_relative_volume(api, ticker: str, current_volume: int, now=None):
    ...
    try:
        ...
        return round(current_volume / baseline, 2), (
            "pre-market volume / 20-day daily baseline (estimate)"
        )
    except Exception as error:
...
def evaluate_ticker(api, ticker, raw_hit=None, **kwargs):
    """Return readable diagnostics for all five pillars."""
    raw_hit = raw_hit or {}
    ...
    checks = {
        "gap": gap is not None and gap >= 10.0,
        "price": price is not None and 2.0 <= price <= 20.0,
        "rel_vol": rel_vol is not None and rel_vol >= 5.0,
    ...
    return {
        ...
        "pillar_details": {
            "gap": (
                f"{gap:.1f}% versus previous regular close"
                if gap is not None else "unavailable"
            ),
            "price": (
                f"${price:.2f} in $2-$20"
            ...
            "rel_vol": (
                f"{rel_vol:.2f}x >= 5x ({rel_reason})"
                if rel_vol is not None else f"UNKNOWN ({rel_reason})"
            ),
            "volume": (
                f"{volume:,} from {raw_hit.get('premarket_start')} "
                f"to {raw_hit.get('premarket_end')}"
            ),
            "float": (
                f"{int(float_shares):,} <= 20,000,000"
