...
def score_ticker(
    ...
    # Alpaca doesn't give 50-day average volume directly in the free API,
    # so we use a rough approximation: average daily vol from recent bars
    try:
    ...
    # Float: Alpaca doesn't expose float directly — yfinance as fallback
    ...
    # Need >= 4 definitive passes (or 3 passes + 1 unknown where unknown could be passing)
...
def _get_float_yfinance(ticker: str) -> int | None:
    ...
    yfinance returns 'floatShares' which is true float, not shares outstanding.
    """
    try:
