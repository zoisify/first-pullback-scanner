...
Detects the "first pullback" entry signal on 1-minute bars.
Matches Ross Cameron's transcript exactly:
- Entry: first pullback after squeeze, crossing candle trigger
- Stop: below pullback low
- Exit: ONLY on exit indicators (no fixed profit target)
- Runners: hold through strength until tape says stop

Entry rules (from transcript):
1. Stock squeezes >= 5% off swing low
2. Pullback retraces <= 50% of up-leg
3. Pullback stays above 9 EMA AND VWAP
4. Volume dries up on red pullback candles
5. "Crossing candle": first candle whose HIGH > prior candle's HIGH
-> Enter on break; stop = pullback low

Exit indicators (Ross's exact rules, systematized):
1. Stop hit (hard stop below pullback low)
2. Volume spike on red candle (proxy for "big seller on Level 2")
3. Topping tail candle (large upper wick, closes red)
4. Price closes below 9 EMA
5. Price closes below VWAP
6. Hard 10:00 AM ET cutoff (Ross's hardest rule)

NO fixed profit target - Ross explicitly says he doesn't cap winners.
"""
