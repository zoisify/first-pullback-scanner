...
def save_scan_log(candidates, all_results):
    ...
    with open(path, "w", newline="") as file:
        ...
        for item in all_results:
            writer.writerow({
                "ticker": item.get("ticker", ""), "score": item.get("score", 0), "price": item.get("price", ""),
                "gap_pct": item.get("gap_pct", ""), "rel_vol": item.get("rel_vol", ""), "total_vol": item.get("total_vol", ""),
                "float": item.get("float", ""), "gap": item.get("pillars", {}).get("gap", ""),
                "price_pillar": item.get("pillars", {}).get("price", ""), "rel_vol_pillar": item.get("pillars", {}).get("rel_vol", ""),
                "volume_pillar": item.get("pillars", {}).get("volume", ""), "float_pillar": item.get("pillars", {}).get("float", ""),
            })
...
def main():
    ...
    if not top_candidate:
        save_scan_log([], results)
        write_watchlist_csv([])
        send_no_candidates(f"Found {len(raw_hits)} gapping stocks. Full five-pillar diagnostics sent, but none passed >=4/5 pillars. No trades today.")
        print("\n Done.")
        return

    save_scan_log(top_candidate, results)
